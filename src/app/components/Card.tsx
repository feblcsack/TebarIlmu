  ...
  return (
    <ul className="grid grid-cols-1 grid-rows-none gap-4 md:grid-cols-12 md:grid-rows-3 lg:gap-4 xl:max-h-[34rem] xl:grid-rows-2">
      <GridItem
        ...
        icon={<Box className="h-4 w-4 text-zinc-300" />}
      ...
      <GridItem
        ...
        icon={<Sparkles className="h-4 w-4 text-zinc-300" />}
...
const GridItem = ({ area, icon, title, description }: GridItemProps) => {
  return (
    <li className={`min-h-[14rem] list-none ${area}`}>
      <div className="relative h-full rounded-2xl border border-zinc-700 bg-zinc-900 p-2 md:rounded-3xl md:p-3 shadow-md">
        <GlowingEffect
          spread={40}
          glow={true}
          disabled={false}
        ...
        <div className="relative flex h-full flex-col justify-between gap-6 overflow-hidden rounded-xl p-6 md:p-6">
          <div className="relative flex flex-1 flex-col justify-between gap-3">
            <div className="w-fit rounded-lg border border-zinc-600 p-2 bg-zinc-800">
              {icon}
            </div>
